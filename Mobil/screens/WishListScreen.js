...
const WishListScreen = ({ navigation }) => {
  ...
  const remove= async(id)=>{
    const response= await ProductService.DeleteWishListById({id:id});
    ...
    <TouchableOpacity style={styles.productItem} onPress={()=>{
      ...
      <View style={styles.productInfo}>
        ...
        <View style={styles.removeButtonContainer}>
            <TouchableOpacity onPress={()=>remove(item.wishListId)}>
  ...
  return (
    ...
    />):(<Text style={styles.infoText}>İstek Listenizde Ürün Bulunmamaktadır</Text>)}
