...
const Products = ({ navigation }) => {
  ...
  const handleNewProduct=()=>{
    navigation.navigate('SaveProduct',{id:0});
  };

  const getProducts=async()=>{
    setIsLoading(true);
    const res= await ProductService.GetProductsForAdmin();
    setIsLoading(false);
    if(res&& !res.error){
  ...
  const getCategories=async()=>{
    ...
    setIsLoading(false)
    if(res&& !res.error){
      setCategories([{label:'Tüm kategoriler',value:0},...res.data]);
    }
  };
  ...
  const productRenderItem = ({item}) => (
    <TouchableOpacity style={styles.productItem} onPress={()=>{
      navigation.navigate('SaveProduct',{id:item.id});
    }}>
  ...
  return (
    ...
    <TouchableOpacity style={styles.saveButton}  onPress={handleNewProduct}>
          <Text style={{color:'white'}}>Yeni Ürün Ekle</Text>
      </TouchableOpacity>
    </View>
    </>)}
